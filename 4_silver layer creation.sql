--Silver layer creation and final insert of transformed data
USE Datawarehouse_Project;

-- DDL Statements for CRM tables with added metadata columns
...
--DDL Statements for ERP tables with added metadata columns
...
/* ============================================================
   ...
   ------------------------------------------------------------
   Layer        : Silver
   Load Type    : Full Refresh (TRUNCATE + INSERT)
   Frequency    : Daily Batch
   Source Layer : Bronze
   Target Layer : Silver

   Purpose:
   - Centralized orchestration of all Silver layer transformations
   - Applies data cleansing, standardization, and enrichment
   - Ensures consistent, analytics-ready intermediate data
   - Enables simple daily execution via a single procedure

   Design Notes:
   - Full refresh strategy is used for deterministic results
   - Execution order respects data dependencies
   - Load duration is logged per table and per batch
   - TRY / CATCH used for controlled failure handling
   ============================================================ */
...
BEGIN
    /* --------------------------------------------------------
       ...
       --------------------------------------------------------
       @start_time        : Tracks execution start per table
       @end_time          : Tracks execution end per table
       @batch_start_time  : Tracks full Silver load start
       @batch_end_time    : Tracks full Silver load end
       -------------------------------------------------------- */
    DECLARE 
        @start_time DATETIME,
        @end_time DATETIME,
